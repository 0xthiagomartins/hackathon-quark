...
class State:
    ...
    def __str__(self):
        ...
        return self.__separator.join(
            str(getattr(self, key)) for key in properties if not key.startswith("__")
        )
    ...
    def connection(self):
        if self.__server is None:
            self.__server = get_in_memory_connection()
    ...
    def get_fields(self, field_names: list):
        ...
        return dict(
            zip(
                field_names,
                [item.decode() if item is not None else None for item in values],
            )
        )
